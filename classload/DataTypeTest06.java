/*
    关于浮点型数据类型：
	   ...
	   为程序员准备了精确度更高的类型，只不过这种类型是一种引用的数据类型，
	   不属于基本数据类型，他是：java.math.BigDecimal

	   其实java程序中SUN提供了一套庞大的类库，java程序员是基于这套基础的
	   类库来进行开发的。所以要知道java的SE类库的字节码在哪儿，要知道java的
	   SE类库的原码在哪。
	       *SE类库字节码：C:\Program Files\Java\jdk1.8.0_151\jre\lib\rt.jar
		   *SE类库原码：C:\Program Files\Java\jdk1.8.0_151\src.zip
         
		 例如：String.java和String.class
		 我们的(String[]args)中的String使用的就是String.class字节码文件。

		 在java语言中，所有的浮点型字面值，默认被当做double类型处理。
		 要想该字面值以float类型来处理，需要在字面值后面添加F/f
		 注意：
		    double和float在计算机内部二进制存储的时候存储的都是近似值。
			在现实世界当中，有一些数据是无限循环的，例如：10/3=3.333333333
			计算机的资源是有限的，用有限的资源去存储无限的数字，只能存储近似值。
